...
# ---------- Новый эндпоинт для HW04: все флаги качества ----------
...
async def quality_flags_from_csv(file: UploadFile = File(...)):
    """
    Эндпоинт возвращает ВСЕ флаги качества, включая новые из HW03.

    В отличие от /quality-from-csv, который возвращает только булевы флаги,
    этот эндпоинт возвращает полный словарь со всеми данными от compute_quality_flags.
    """
    ...
    print(
        ...
        f"shape={df.shape} latency_ms={latency_ms:.1f} ms"
    )

    # Возвращаем ВСЕ флаги, включая не только булевы
    return {
        "filename": file.filename,
        "dataset_shape": {"n_rows": df.shape[0], "n_cols": df.shape[1]},
        "flags": flags_all,  # Все флаги из HW03
        "latency_ms": latency_ms,
        "message": "Полный набор флагов качества успешно получен"
    }
